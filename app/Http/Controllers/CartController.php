...
class CartController extends Controller
{
    ...
    public  function Show(Request  $request)
    {
        ...
        $data_product_follow_user_id = User::find($user_id)->product_cart()->join(
            'products',
            'products.id',
            '=',
    ...
    public  function  Create(Request $request)
    {
        ...
        if ($cart == null) {
            DB::table('user_cart')->insert(['user_id' => $id_user, 'product_id' => $product_id, 'quantity' => $quantity]);
        } else {
            try {
                $cart =  user_cart::where('user_id', '=', $id_user)->where('product_id', '=', $request->product_id)->firstOrFail();
                $UpdateValue = $cart['quantity'] + $quantity;
